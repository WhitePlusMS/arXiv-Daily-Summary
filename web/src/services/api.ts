...
export const initializeService = async (): Promise<ApiResponse<{ initialized: boolean }>> => {
  const response = await api.post('/api/initialize', undefined, { signal: getAbortSignal('POST /api/initialize') })
...
export const getUserProfiles = async (): Promise<ApiResponse<UserProfile[]>> => {
  const response = await api.get('/api/user-profiles', { signal: getAbortSignal('GET /api/user-profiles') })
  return response.data
}

export const getResearchInterests = async (): Promise<ApiResponse<string[]>> => {
  const response = await api.get('/api/research-interests', { signal: getAbortSignal('GET /api/research-interests') })
  return response.data
}

export const updateResearchInterests = async (request: { interests: string[] }): Promise<ApiResponse<{ interests: string[] }>> => {
  const response = await api.post('/api/research-interests', request, { signal: getAbortSignal('POST /api/research-interests') })
  return response.data
}

export const initializeComponents = async (request: { profile_name: string }): Promise<ApiResponse<{ initialized: boolean }>> => {
  const response = await api.post('/api/initialize-components', request, { signal: getAbortSignal('POST /api/initialize-components') })
  return response.data
}
...
export const runRecommendation = async (request: { profile_name: string, debug_mode: boolean }): Promise<ApiResponse<RecommendationResult>> => {
  const response = await api.post('/api/run-recommendation', request, { timeout: 300000, signal: getAbortSignal('POST /api/run-recommendation') })
  return response.data
}

export const getRecentReports = async (): Promise<ApiResponse<ReportItem[]>> => {
  const response = await api.get('/api/recent-reports', { signal: getAbortSignal('GET /api/recent-reports') })
  return response.data
}

// 获取分类数据
export const getCategories = async (): Promise<ApiResponse<Category[]>> => {
  const response = await api.get('/api/categories', { signal: getAbortSignal('GET /api/categories') })
  return response.data
}
...
export const getEnvConfig = async (): Promise<ApiResponse<Record<string, unknown>>> => {
  const response = await api.get('/api/env-config', { signal: getAbortSignal('GET /api/env-config') })
  return response.data
}

export const saveEnvConfig = async (request: { config: Record<string, unknown> }): Promise<ApiResponse<{ saved: boolean }>> => {
  const response = await api.post('/api/env-config/save', request, { signal: getAbortSignal('POST /api/env-config/save') })
  return response.data
}

export const reloadEnvConfig = async (): Promise<ApiResponse<Record<string, unknown>>> => {
  const response = await api.post('/api/env-config/reload', undefined, { signal: getAbortSignal('POST /api/env-config/reload') })
  return response.data
}

export const restoreDefaultEnvConfig = async (): Promise<ApiResponse<Record<string, unknown>>> => {
  const response = await api.post('/api/env-config/restore-default', undefined, { signal: getAbortSignal('POST /api/env-config/restore-default') })
  return response.data
}
...
export const getMatcherProviderConfig = async (): Promise<ApiResponse<Record<string, unknown>>> => {
  const response = await api.get('/api/matcher/provider', { signal: getAbortSignal('GET /api/matcher/provider') })
  return response.data
}
...
export const getMatcherData = async (): Promise<ApiResponse<UserProfile[]> & { stats?: Record<string, unknown> }> => {
  const response = await api.get('/api/matcher/data', { signal: getAbortSignal('GET /api/matcher/data') })
  return response.data
}

// 优化研究描述
export const optimizeMatcherDescription = async (request: { user_input: string }): Promise<ApiResponse<{ optimized: string }>> => {
  const response = await api.post('/api/matcher/optimize', request, { signal: getAbortSignal('POST /api/matcher/optimize') })
  return response.data
}
...
export const runCategoryMatching = async (request: { user_input: string, username: string, top_n: number }): Promise<ApiResponse<{ results: { id: string, name: string, score: number }[], token_usage: { input_tokens: number, output_tokens: number, total_tokens: number } }>> => {
  const response = await api.post('/api/matcher/run', request, { timeout: 300000, signal: getAbortSignal('POST /api/matcher/run') })
  return response.data
}
...
export const updateMatcherRecord = async (request: { index: number, username: string, category_id: string, user_input: string }): Promise<ApiResponse<{ updated: boolean }>> => {
  const response = await api.put('/api/matcher/record', request, { signal: getAbortSignal('PUT /api/matcher/record') })
  return response.data
}

// 删除单条匹配记录
export const deleteMatcherRecord = async (params: { index: number }): Promise<ApiResponse<{ deleted: boolean }>> => {
  const response = await api.delete('/api/matcher/record', { params: { index: params.index }, signal: getAbortSignal('DELETE /api/matcher/record') })
  return response.data
}

// 批量删除匹配记录
export const batchDeleteMatcherRecords = async (request: { indices: number[] }): Promise<ApiResponse<{ deleted: number }>> => {
  const response = await api.delete('/api/matcher/records', { data: request, signal: getAbortSignal('DELETE /api/matcher/records') })
  return response.data
}

// 列出评分文件
export const listMatcherScoreFiles = async (): Promise<ApiResponse<string[]>> => {
  const response = await api.get('/api/matcher/scores', { signal: getAbortSignal('GET /api/matcher/scores') })
  return response.data
}

// 读取评分文件内容
export const readMatcherScoreFileContent = async (params: { name: string }): Promise<ApiResponse<{ name: string, content: string }>> => {
  const response = await api.get('/api/matcher/scores/content', { params, signal: getAbortSignal('GET /api/matcher/scores/content') })
  return response.data
}
...
export const deleteMatcherScoreFile = async (params: { name: string }): Promise<ApiResponse<{ deleted: boolean }>> => {
  const response = await api.delete('/api/matcher/scores', { params, signal: getAbortSignal('DELETE /api/matcher/scores') })
  return response.data
}
